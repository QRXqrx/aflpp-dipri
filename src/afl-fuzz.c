...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (likely(!afl->stop_soon)) {
    ...
    // When enter this branch:
    // 1. not use old seed selection, and
    //        runs_in_current_cycle exceeds number of queue inputs
    // 2. or, use old seed selection and current queue entry is null.
    if (unlikely((!afl->old_seed_selection &&
                  runs_in_current_cycle > afl->queued_items) ||
                 (afl->old_seed_selection && !afl->queue_cur))) {

      // Check if using syn-mode
      ...
      if (unlikely(afl->old_seed_selection)) {
        ...
        // If no such entry is found, return back to the first
        if (afl->current_entry >= afl->queued_items) { afl->current_entry = 0; }

        // Locate current queue entry with respect to entry-id.
        afl->queue_cur = afl->queue_buf[afl->current_entry];

        // Don't know what is seek_to used for
        if (unlikely(seek_to)) {
    ...
    /* End of Non-vanilla seed selection */
